...
Due to the ambiguity in the relationship between laurasithians and the placement of tree shrews, we also inferred gene trees using MrBayes [@Ronquist2012; @Fan2013; @Chen2017]. To infer each gene tree, we selected the General Time Reversible (GTR) substitution model with gamma-distributed rate variation across sites and the Markov chain Monte Carlo (mcmc) sampling was run until the standard deviation frequency was less than 0.01 [@Ronquist2012]. We used this approach  to account for effects of incomplete lineage sorting (ILS) [@Pamilo1988; @Rosenberg2002; @Scornavacca2017].  Using gene trees and using the consensus species tree produced highly similar results (Table S4).
